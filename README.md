...
When you are ready to compile your book to HTML, use:

```bash
pretext build html
```

To view the compiled book on `localhost`, use:

```bash
pretext view
```

To compile Markdown (or LaTeX, etc) into pretext, clone [Oscar Levin's Lua-based converter](https://github.com/oscarlevin/pandoc-pretext) into the `custom` subdirectory of Pandoc's *User Data Directory*, which can be viewed using `pandoc --version.` Then, use:

```bash
pandoc {filename}.md -t pretext.lua -o {filename}.ptx
```
